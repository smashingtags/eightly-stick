...
// Eight.ly Forge layout: agent/ lives under Shared/. Data + chats + env go
// under Shared/chat_data/agent/ so fast-chat history stays separate from
// agent sessions. Bundled tools (node, git) live under Shared/tools/.
const SHARED_DIR = join(__dirname, '..');
const ROOT_DIR = join(SHARED_DIR, '..');
const DATA_DIR = join(SHARED_DIR, 'chat_data', 'agent');
...
const TOOLS_DIR = join(SHARED_DIR, 'tools');
const BIN_DIR = TOOLS_DIR;
// FastChatUI stays on 3333; the agent dashboard sits on 3334 in the Forge
// port plan. Override via ELY_AGENT_PORT env if you need to.
const PORT = Number(process.env.ELY_AGENT_PORT || 3334);
